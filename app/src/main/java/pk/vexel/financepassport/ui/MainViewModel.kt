...
class MainViewModel(private val repository: FinanceRepository) : ViewModel() {
    ...
    private fun operation(block: suspend () -> Unit) = viewModelScope.launch {
        runCatching { block() }.onFailure { errorMessage = "Could not save changes. Check the form and try again." }
    }

    fun clearError() { errorMessage = null }
    fun addAccount(name: String, type: String, openingBalanceMinor: Long) = operation { repository.addAccount(name, type, openingBalanceMinor) }
    fun accountMovement(accountId: String) = repository.accountMovement(accountId)
    fun updateAccount(id: String, name: String, openingBalanceMinor: Long) = operation { repository.updateAccount(id, name, openingBalanceMinor) }
    fun archiveAccount(id: String) = operation { repository.archiveAccount(id) }
    fun addEvent(type: FinancialEventType, amountMinor: Long, accountId: String, description: String, category: String? = null) = operation { repository.addEvent(type, amountMinor, accountId, description, category) }
    fun addRecurringItem(context: android.content.Context, title: String, type: FinancialEventType, amountMinor: Long, accountId: String, category: String?, frequency: String, delayDays: Long) = operation { repository.addRecurringItem(context, title, type, amountMinor, accountId, category, frequency, delayDays) }
    fun pauseRecurringItem(context: android.content.Context, id: String) = operation { repository.pauseRecurringItem(context, id) }
    fun transfer(source: String, destination: String, amountMinor: Long, description: String) = operation { repository.transfer(source, destination, amountMinor, description) }
    fun addAsset(title: String, valueMinor: Long) = operation { repository.addAsset(title, "OTHER", valueMinor) }
    fun updateAssetValuation(id: String, valueMinor: Long) = operation { repository.updateAssetValuation(id, valueMinor) }
    fun disposeAsset(id: String, valueMinor: Long) = operation { repository.disposeAsset(id, valueMinor) }
    fun addLiability(title: String, valueMinor: Long) = operation { repository.addLiability(title, "OTHER", valueMinor) }
    fun recordLiabilityPayment(id: String, valueMinor: Long) = operation { repository.recordLiabilityPayment(id, valueMinor) }
    fun addInvestmentEvent(security: String, type: String, amountMinor: Long, quantityMinor: Long = 0) = operation { repository.addInvestmentEvent(security, type, amountMinor, quantityMinor) }
    fun addReceivable(title: String, counterparty: String, amountMinor: Long) = operation { repository.addReceivable(title, counterparty, amountMinor) }
    fun recordReceivablePayment(id: String, amountMinor: Long) = operation { repository.recordReceivablePayment(id, amountMinor) }
    fun addGoal(title: String, targetMinor: Long) = operation { repository.addGoal(title, targetMinor) }
    fun addOfficialRecord(type: String, title: String, identifier: String?) = operation { repository.addOfficialRecord(type, title, identifier) }
    fun addCalendarItem(context: android.content.Context, title: String, kind: String, delayMinutes: Long) = operation { repository.addCalendarItem(context, title, kind, delayMinutes) }
    fun updateCalendarStatus(context: android.content.Context, id: String, status: String) = operation { repository.updateCalendarStatus(context, id, status) }
    fun rescheduleCalendarItem(context: android.content.Context, id: String, delayMinutes: Long) = operation { repository.rescheduleCalendarItem(context, id, delayMinutes) }
    fun deleteAllData() = operation { repository.deleteAllData() }
